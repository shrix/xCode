...
# # Detect x-axis position of the sports ball
# def detect_x_pos(detections, yolo_model, frame_width):
#     for detection in detections:
#         class_id = int(detection[5])
#         if yolo_model.names[class_id] == "sports ball":
#             x_center = (detection[0] + detection[2]) / 2  # Average of left and right x-coordinates
#             return int(x_center)
#     return None  # Return None if no sports ball is detected
...
def process_video(input_vid: str, output_vid: str = None) -> None:
    ...
    while True:
        ...
        if detections:
            ...
            # # Detect x-position of the sports ball
            # ball_x_position = detect_x_pos(detections, yolo_model, seg_frame_size[0])
            # if ball_x_position is not None:
            #     print(f"Sports ball x-position: {ball_x_position}")
            #     if prev_ball_x is not None:
            #         current_direction = 1 if ball_x_position > prev_ball_x else -1 if ball_x_position < prev_ball_x else 0
            #         if prev_direction is not None and current_direction != prev_direction and current_direction != 0:
            #             direction_change_frames.append(frames_processed - 1)
            #             print(f"Ball changed direction at frame {frames_processed - 1}")
            #         prev_direction = current_direction
            #     prev_ball_x = ball_x_position
    ...
    # print(f"> Frames where ball changed direction: {direction_change_frames}")
