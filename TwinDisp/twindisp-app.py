...
def main():
    ...
    if len(sys.argv) != 3:
    # for python twindisp.py ...
    #     print("Usage: python twindisp.py <video1_path> <video2_path>")
    #     sys.exit(1)
    # video1_path = sys.argv[1]
    # video2_path = sys.argv[2]

    # for twindisp mac app ...
    # chmod +x TwinDisp.app/Contents/MacOS/*
    # cp -R TwinDisp.app /Applications/
    # mv TwinDisp.app /Applications/
        video1_path, _ = QFileDialog.getOpenFileName(None, "Select first video file", "", "Video Files (*.mp4 *.avi *.mov)")
        if not video1_path:
            print("No video selected. Exiting.")
            sys.exit(1)
        
        video2_path, _ = QFileDialog.getOpenFileName(None, "Select second video file", "", "Video Files (*.mp4 *.avi *.mov)")
        if not video2_path:
            print("No video selected. Exiting.")
            sys.exit(1)
    else:
